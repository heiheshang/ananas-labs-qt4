 ...
 *
 */
CfgForm::CfgForm(const QString & title, QWidget * parent, Qt::WindowFlags flags)
...
void
set_Icon(QListWidgetItem *item, const char *name)
{
    char in[200];
    strcpy(in, name);
    strcat(in,".png");
    item->setIcon(QIcon(rcIcon(in)));
}
...
void
CfgForm::init(DomCfgItem *incfg,bool flagNew )	//	flagNew shows if it needs to create new configuration
{
    //statusBar()->hide();
    configform=this;
	...
	//} 
    cfg = incfg;
    setCaption(tr("Business scheme: ")+cfg->attr(mda_name));
    //QGridLayout *l = new QGridLayout( tabWidget->page(0), 1, 1, 1, // 2, "tablayout" );
	configTreeView *mdtree = new configTreeView(cfg,this);
        setContextMenuPolicy(Qt::CustomContextMenu);
    	setWidget(mdtree);
	setMinimumSize( QSize( 1, 1 ) );
	mdtree->setMinimumSize( QSize( 1, 1 ) );
...
//     initImageCollection();	//	image collection tab initialization
       ...
       connect(mainform->actionPreferences, SIGNAL(activated()), mdtree, SLOT( slotOptions() ) );
// 
...
{
//     DomCfgItem *oroot, *image_collection, *image;
//     int i, n;
//     QPixmap pix;
//     QString name;
// 
//     //oroot = cfg->root();	// get root
//     image_collection = cfg->root()->find(md_image_collection);	// get image col part
//     if ( image_collection!=0 ) cfg->insert( cfg->root(), md_image_collection, QString::null, -1 );
//     n = image_collection->childCount();	// count images
//     for ( i = 0; i < n; i++ ) {
// 	image = image_collection->child(i);	// foreach
// 	name = image->attr(mda_name );	// get name
// 	pix.loadFromData(image->binary());	// load binary
...
// 	//vImageCollection->addItem( item );	// insert image
...
// 		QFileInfo fi( name );
// 		name = fi.baseName();
// 		QListWidgetItem *item = new QListWidgetItem( vImageCollection);
// 		item->setText(name);
...
void CfgForm::save()
{
	QFile file(db->values["configfile"]);
aLog::print(aLog::Debug,"void CfgForm::save() "+db->values["configfile"]+"\n");
   	QByteArray buf( cfg->root()->node().toDocument().toString(4).utf8() );
    	if ( file.open( QIODevice::WriteOnly ) ) {
	QTextStream ts( &file );
	cfg->root()->node().save(ts, 4);
	file.close();
...
void CfgForm::listCfg_onItem( QListWidgetItem * )
{
