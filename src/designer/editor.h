...
public:
	...
	Editor(TabWidget *parent, configTreeView *mainimpl, InitCompletion *completion, QString name="");
	~Editor();
	bool open(bool silentMode);
	bool save();
	bool close();
	void setModule(QDomNode &module) { m_moduletext = module; m_textEdit->document()->setModified(true);};
	QDomNode moduletext() { return m_moduletext; };
...
public slots:
	...
	void setEndLine(configTreeView::EndLine end) { m_textEdit->setEndLine(end); };
...
private:
	...
	configTreeView *m_mainimpl;
	QWidget *m_findWidget;
	QWidget *m_replaceWidget;
	TextEdit *m_textEdit;
	QDomNode m_moduletext;
