...
void LineNumbers::paintEvent( QPaintEvent* )
{
	...
	for ( QTextBlock block = m_textEdit->document()->begin(); block.isValid(); block = block.next(), m_lineNumber++ )
	{
		...
		{
			QPixmap pixmap = QPixmap(":/images/breakpoint.png");
			if( !blockUserData->breakpointCondition.isEmpty() )
				pixmap = QPixmap(":/images/breakpoint2.png");
			p.drawPixmap( 1, qRound( position.y() ) -contentsY-4, pixmap);
		}
		if( blockUserData && blockUserData->bookmark )
		{
			p.drawPixmap( 3, qRound( position.y() ) -contentsY-6,QPixmap(":/images/bookmark.png"));
		}
		if( blockUserData && !blockUserData->errorString.isEmpty() )
		{
			p.drawPixmap( 3, qRound( position.y() ) -contentsY-2,QPixmap(":/images/error.png") );
		}
		else if( blockUserData && !blockUserData->warningString.isEmpty() )
		{
			p.drawPixmap( 3, qRound( position.y() ) -contentsY-2,QPixmap(":/images/warning.png") );
		}
	}
	p.end();
}
...
void LineNumbers::mousePressEvent ( QMouseEvent * event )
{
	...
	if( event->button() == Qt::RightButton )
	{
		...
		connect(menu->addAction(QIcon(":/images/bookmark.png"), tr("Toogle Bookmark")), SIGNAL(triggered()), this, SLOT(slotToggleBookmark()) );
		connect(menu->addAction(QIcon(":/images/breakpoint.png"), tr("Toogle Breakpoint")), SIGNAL(triggered()), this, SLOT(slotToggleBreakpoint()) );
		//
		BlockUserData *blockUserData = (BlockUserData*)m_cursor.block().userData();
		if( blockUserData && blockUserData->breakpoint )
		{
			menu->addSeparator();
			connect(menu->addAction(QIcon(":/images/breakpoint2.png"), tr("Breakpoint Condition...")), SIGNAL(triggered()), this, SLOT(slotBreakpointCondition()) );
		}
		//
