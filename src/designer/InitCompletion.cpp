...
{
    QStringList list = QDir( QDir::tempPath() ).entryList(QStringList() << "ananas-completion-*", QDir::Files);
    ...
    if ( m_stopRequired )
    {
        if ( !connectQAnanasDB( getQAnanasPath() + "ananas.db" ) )
        {
            return;
        }
        createTables();
        QSqlQuery query(QSqlDatabase::database(getQAnanasPath() + "ananas.db" ));
...
void InitCompletion::slotInitParse(InitCompletion::Request request, QString filename, const QString &text, bool showAllResults, bool emitResults, bool /*showDuplicateEntries*/, QString name)
{
    ...
    tagsIncludesPath = Path + '/' + "ananas-completion-" + QFileInfo(filename).baseName() + "-tags_includes";
    tagsFilePath = Path + '/' + "ananas-completion-" + QFileInfo(filename).baseName() + "-tags";
    smallTagsFilePath = Path + '/' + "annas-completion-" + QFileInfo(filename).baseName() + "-small-tags";
    parsedFilePath = Path + '/' + "annas-completion-" + QFileInfo(filename).baseName() + "-parsed_file";
...
void InitCompletion::run()
{
    if ( m_request == CheckQtDatabase )
    {
        ...
        if ( list.count() )
        {
			QSqlDatabase::removeDatabase(getQAnanasPath() + "ananas.db");
            return;
        }
        populateQtDatabase();
		QSqlDatabase::removeDatabase(getQAnanasPath() + "ananas.db");
...
//
bool InitCompletion::connectQAnanasDB(QString const& dbName)
{
    ...
    if (!database.open())
    {
        emit showMessage(	  QObject::tr("Unable to establish a database connection.")+"\n"+
                              QObject::tr("Ananas needs SQLite support. Please read "
...
void InitCompletion::createTables()
{
    QString queryString = "create table tags ("
                          ...
                          "isStatic int"
                          ")";
    QSqlQuery query(QSqlDatabase::database(getQAnanasPath() + "qdevelop.db" ));
    query.exec(queryString);
    //
...
{
   QString command = ctagsCmdPath + " -L "+QDir::tempPath()+"/ananasfilenames -f \"" + QDir::tempPath()+"/qttags" +
    ...
    foreach(QString incDir, m_qtInclude)
    {
     ...
     QFile f(QDir::tempPath()+"/ananasfilenames");
