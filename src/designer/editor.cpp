...
Editor::Editor(TabWidget * parent, configTreeView *mainimpl, InitCompletion *completion, QString name)
        : QWidget(parent)
{
    m_parent = parent;
    m_mainimpl = mainimpl;
...
//             //
	     ...
	     m_comboMethods->setLineEdit( new LineEdit(m_comboClasses) );
             ...
             m_comboMethods->setObjectName(QString::fromUtf8("comboMethodes"));             m_comboMethods->lineEdit()->setAlignment(Qt::AlignLeft);
...
{
    bool ret = m_textEdit->open(silentMode, m_moduletext, m_lastModified);
    //if ( ret && (suffixe( m_filename ).toLower() == "cpp" || suffixe( m_filename ).toLower() == "cc"))
    //    slotClassesMethodsList();
    QByteArray array( m_textEdit->toPlainText().toUtf8() );
    if ( array.count() )
    {
        char *ptr = array.data();
        quint16 check = qChecksum(ptr, array.length());
...
bool Editor::close()
{
    bool ret = m_textEdit->close( m_moduletext );
    if ( ret )
        clearAllBookmarks();
    return true;
}
...
{
	//return true;
    return m_textEdit->save( m_moduletext, m_lastModified);
}
//
