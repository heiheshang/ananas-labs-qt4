...
TextEdit::TextEdit(Editor * parent, configTreeView *mainimpl, InitCompletion *completion)
        : QTextEdit(parent), m_editor(parent), m_mainImpl(mainimpl), m_completion(completion), m_mouseHidden(false)
{
    ...
    m_matchingBegin = -1;
    m_matchingEnd = -1;
    m_endLine = configTreeView::Default;
...
void TextEdit::completeCode()
{
    ...
    {
    	QMessageBox::warning(m_mainImpl, "Ananas", tr("The Qt database building is in progress.\nTry to complete code later."));
        return;
   	}
    if ( !m_completion )
        return;
    ...
    emit initParse(InitCompletion::Completion, "ananas", c, true, true, false, QString());
}
    ...
    if ( tagList.count() )
    {
        ...
        foreach(Tag tag, tagList)
        {
            ...
            if ( tag.kind == "function" || tag.kind == "prototype")
                item->setIcon(QIcon(":/images/CV"+tag.access+"_meth.png"));
            else if ( tag.kind == "member" )
                item->setIcon(QIcon(":/images/CV"+tag.access+"_var.png"));
            else if ( tag.kind == "struct" )
                item->setIcon(QIcon(":/images/CVstruct.png"));
            else if ( tag.kind == "class" )
                item->setIcon(QIcon(":/images/CVclass.png"));
...
//
bool TextEdit::open(bool silentMode, QDomNode &moduletext, QDateTime &lastModified)
{
...
//     }
    ...
    setPlainText(moduletext.toElement().text());
    ...
    if ( m_lineNumbers )
     ...
     {
         emit initParse(InitCompletion::Completion, "ananas", toPlainText(), true, false, false, QString());
     }
...
bool TextEdit::close(QDomNode &node)
{
    if ( document()->isModified() )
    {
        // Proposer sauvegarde
        int rep = QMessageBox::question(this, tr("Ananas"),
                                        tr("Save \"%1\"").arg(tr(node.nodeName())), tr("Yes"), tr("No"), tr("Cancel"), 0, 2 );
        if ( rep == 2 )
            return false;
        if ( rep == 0 )
        {
            QDateTime info;
            return save(node, info);
        }
    }
    return true;
}
//
bool TextEdit::save(QDomNode &node, QDateTime &lastModified)
{
qDebug() << "not yet isModified()";
    if ( !document()->isModified() ) {
	qDebug() << "isModified()" << document()->isModified();
        return true;
	}
//     QFile file( filename );
...
//     }
    ...
    if ( m_endLine != configTreeView::Default )
    {
        s.replace("\r\n", "\n");
        if ( m_endLine == configTreeView::Windows )
            s.replace("\n", "\r\n");
    }
    //int mib = m_mainImpl->mibCodec();
    //QTextCodec *codec = QTextCodec::codecForMib(mib);
...
void TextEdit::keyPressEvent ( QKeyEvent * event )
{
    ...
    else if ((event->key() == Qt::Key_Up) && (QApplication::keyboardModifiers().testFlag(Qt::ControlModifier)))
    {
...
void TextEdit::completionHelp()
{
    ...
    emit initParse(InitCompletion::Completion, "ananas", c, true, true, true, name);
}
