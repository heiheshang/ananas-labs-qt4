...
public:
	...
	void saveAsTemp();
    TextEdit(Editor * parent, configTreeView *mainimpl, InitCompletion *completion);
    ~TextEdit();
    bool open(bool silentMode, QDomNode &textmodule, QDateTime &lastModified);
    bool save(QDomNode &node, QDateTime &lastModified);
    bool close(QDomNode &node);
    ...
    void setEndLine(configTreeView::EndLine end)
    {
        m_endLine = end;
    };
...
private:
    ...
    void match();
    void clearMatch();
    configTreeView::EndLine m_endLine;
    bool m_tabSpaces;
