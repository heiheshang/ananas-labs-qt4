...
aCfg::find(long id)
{
	long idl = id;
	aCfgItem i;
	QHash<long, aCfgItemContaner*>::const_iterator ic;
	//aCfgItemContaner *ic;
	...
	ic = idcache.find( idl );
	while (ic != idcache.end() && ic.key() == idl) {
     		i=ic.value()->item;
     		++ic;
 	}
	//if (ic) i = ic->item(ic);
...
aCfg::setBinary( aCfgItem context, const QByteArray &value, const QString &format )
{
	...
	if ( !format.isNull() ) ( context, mda_format, format );
}
...
// DomCfgItem *DomCfgItem::parentObject()
// {
// 	DomCfgItem *parentCfg=0;
// 	parentCfg = parent();
// qDebug() << "DomCfgItem::parentObject() " << parentCfg->nodeName();
// 	if (parentCfg==0)
// 		return 0;
// 	while ( parentCfg->nodeName()!=md_document &&
// 		parentCfg->nodeName()!=md_resources &&
// 		parentCfg->nodeName()!=md_aregister &&
// 		parentCfg->nodeName()!=md_iregister &&
// 		parentCfg->nodeName()!=md_catalogue &&
// 		parentCfg->nodeName()!=md_aregisters &&
// 		parentCfg->nodeName()!=md_iregisters)
// 	{
// 		parentCfg = parentCfg->parent();
// 		if (parentCfg==0)
// 			return 0;
// 		
// 	}
...
DomCfgItem *DomCfgItem::child(int i)
{
 DomCfgItem *childItem;
    if (childItems.contains(i))
        return childItems[i];
//aLog::print(aLog::Debug,"row "+QString::number(i));
...
//aLog::print(aLog::Debug,"image");
	...
	while(!cur.isNull()) {
		if (cur.nodeName()==md_string_view) {
			QDomNode curS = cur.firstChild();
			while (!curS.isNull()) {
				if (curS.nodeName()==md_svfunction) {
					childItem = new DomCfgItem(curS, 0, this);
...
{
aLog::print(aLog::Debug,f+" = "+nodeName()+"\n");
 	if (nodeName()==f)
		return this;
	for(int i=0;i<childCount();i++) {
//	aLog::print(aLog::Debug,"child "+f+" = "+child(i)->nodeName()+"\n");
...
QString DomCfgItem::getNameByType(QStringList type)
{
	...
	if (type.at(1)=="O") {
		...
		return QObject::tr(item->configName());
	}
	if (type.at(1)=="N") 
...
DomCfgItem::nextID()
{
	...
	aLog::print(aLog::Info,"До "+last_id.nodeValue());
		id = last_id.nodeValue().toLong();
		if (id == 0 ) id = context_startid;
			++id;
		last_id.setNodeValue(QString("%1").arg(id));
	aLog::print(aLog::Info,"После "+last_id.nodeValue());
	}
	return id;
}
...
{
	DomCfgItem *i;
aLog::print(aLog::Debug,"DomCfgItem::setSText "+nodeName()+"\n");
	i = child(subname);
	if ( i==0) {
		//i = insert(subname);
	}
...
{
    //if (domNode.nodeName()==md_popupmenu)
    //    return false;
    if (domNode.hasChildNodes())
        return true;
    return false;
...
//aLog::print(aLog::Debug,childNode.nodeName());
//aLog::print(aLog::Debug,"childItems A "+childNode.childNodes().item(0).nodeValue());
...
DomCfgItem* DomCfgItemInterfaces::child(QString f)
{
 for (int i=0;i<childCount();i++)
	{
//aLog::print(aLog::Debug,"DomCfgItemInterfaces::child(QString f) "+child(i)->nodeName()+"\n");
	if (child(i)->nodeName()==f)
			return child(i);
	}
return 0;
...
DomCfgItem* DomCfgItemActions::child(QString f)
{
 for (int i=0;i<childCount();i++)
	{
//aLog::print(aLog::Debug,"DomCfgItemActions::child(QString f) "+child(i)->nodeName()+"\n");
	if (child(i)->nodeName()==f)
		return child(i);
	}
return 0;
...
//DomCfgItemActions *DomCfgItemActions::findObjectById(int id)
//{
// return findObjectById(QString::number(id));
//}
//DomCfgItemActions *DomCfgItemActions::findObjectById(QString id)
//{
//	if (root()->hashId.contains(id)) 
//		return static_cast<DomCfgItemActions*>(root()->hashId.value(id));
//return 0;
//}

//DomCfgItemActions *DomCfgItemActions::root()
//{
//	return static_cast<DomCfgItemActions*>(rootNode);
//	
//}	