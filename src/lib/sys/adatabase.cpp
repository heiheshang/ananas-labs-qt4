...
qds_fields ( DomCfgItem *cfg )
{
	...
	*/
	...
	{
		item = cfg->child(j);
		id = item->attr( mda_id ).toInt();
		if ( id )
		{
			t = item->attr ( mda_type );
			if ( !t.isEmpty() )
			{
				if ( t[0]!=' ' )
				{
					dd<<qds_field ( QString ( "uf%1" ).arg ( id ), t, item->attr ( mda_name ) );
//aLog::print(aLog::Debug,qds_field ( QString ( "uf%1" ).arg ( id ), t, item->attr ( mda_name ) ));
...
qds_dd ( DomCfgItem *cfg )
{
	...
	DomCfgItem *gcont, *cont, *item;
	long id;
	int n, i;

	gcont = cfg->root()->find(md_catalogues);
	n = gcont->childCount();
	for ( i = 0; i<n; i++ )
	{
		item = gcont->child( i );
		if ( item!=0 ) 
		{
			...
			cont = item->child(j);
			if ( cont->nodeName()==md_element )
			{
			//for(int e=0;e<cont->childCount();e++) {
			 //if (cont->child(e)->nodeName()==md_field) {
				dd<<QString ( "T=%1|Elements:%2|U" ).arg ( aDatabase::tableDbName ( cont)).arg ( item->attr( mda_name ));
				dd<<"F=id|ID|L|0|0|NP";
...
//aLog::print(aLog::Debug,QString ( "T=%1|Elements:%2|U" ).arg ( aDatabase::tableDbName (cont)).arg ( item->attr(mda_name)));
				qds_fields (cont);}
			//}
			
		
			//cont = cfg.findChild ( item, md_group, 0 );
			if ( cont->nodeName()==md_group )
			{
				dd<<QString ( "T=%1|Groups:%2|U" ).arg ( aDatabase::tableDbName ( cont)).arg ( item->attr ( mda_name ) );
				dd<<"F=id|ID|L|0|0|NP";
				dd<<"F=df|Mark deletede flag|C|1|0|";
				...
				dd<<"I=I3|Index 3|idp|";
//aLog::print(aLog::Debug,QString ( "T=%1|Groups:%2|U" ).arg ( aDatabase::tableDbName (cont)).arg ( item->attr(mda_name)));
				qds_fields ( cont );
			}
		}
	}
	...
	DomCfgItem *rcont, *tcont;
	int tn, ti;

	rcont = cfg->root()->find(md_documents);
	n = rcont->childCount();
//aLog::print(aLog::Debug,"Документы "+QString::number(n));
	for ( i = 0; i<n; i++ )
	{
		item = rcont->child(i);
//aLog::print(aLog::Debug,item->nodeName());
		if ( item!=0 )
		{
		if (item->nodeName()!=md_document) continue;
		 for (int j=0;j<item->childCount();j++){
			tcont = item->child(j);
//aLog::print(aLog::Debug,tcont->nodeName());
			if ( tcont->nodeName()==md_header )
			{
				dd<<QString ( "T=%1|%2|U" ).arg ( aDatabase::tableDbName (tcont ) ).arg ( item->attr ( mda_name ) );
				dd<<"F=id|ID|L|0|0|NP";
//aLog::print(aLog::Debug,QString ( "T=%1|%2|U" ).arg ( aDatabase::tableDbName (tcont)).arg ( item->attr(mda_name)));
				qds_fields ( tcont );
			}
			//tcont = cfg.find ( item, md_tables, 0 );
			if (tcont->nodeName()==md_tables) {
			tn = tcont->childCount();
			DomCfgItem *table;
			for ( ti = 0; ti < tn; ti++ )
			{
				table = tcont->child(ti);
				if ( table!=0 )
				{
					dd<<QString ( "T=%1|Groups:%2|U" ).arg ( aDatabase::tableDbName (table)).arg (tcont->attr ( mda_name ));
					dd<<"F=id|ID|L|0|0|NP";
					dd<<"F=idd|Document ID|L|0|0|";
					dd<<"F=ln|Line number|L|0|0|";
					dd<<"I=I1|Index 1|idd|";
					dd<<"I=I2|Index 2|ln|";
//aLog::print(aLog::Debug,QString ( "T=%1|Groups:%2|U" ).arg ( aDatabase::tableDbName (table)).arg ( tcont->attr(mda_name)));
					qds_fields ( table);
				}
			}
		}//md_tables
	...
	QString filds;
//aLog::print(aLog::Debug,"Регистр оборотный");
	rcont = cfg->root()->find(md_iregisters);
	n = rcont->childCount ();
	for ( i = 0; i<n; i++ )
	{
		item = rcont->child( i );
//aLog::print(aLog::Debug,item->nodeName());
		if (item!=0)
		{
			dd<<QString ( "T=%1|%2|U" ).arg ( aDatabase::tableDbName(item)).arg ( item->attr(mda_name));
			dd<<"F=id|ID|L|0|0|NP";
			dd<<"F=idd|Document ID|L|0|0|";
			...
			dd<<"I=I3|Index 3|ln|";
//aLog::print(aLog::Debug,QString ( "T=%1|%2|U" ).arg ( aDatabase::tableDbName (item)).arg (item->attr(mda_name)));
			qds_fields ( item );
		}
	}
	...
	DomCfgItem *res, *dim;
//aLog::print(aLog::Debug,"Регистр накопительный");
	rcont = cfg->root()->find(md_aregisters);
	n = rcont->childCount( );
	for ( i = 0; i<n; i++ )
	{
		item = rcont->child(i);
...
//aLog::print(aLog::Debug,QString ( "T=%1|%2|U" ).arg ( aDatabase::tableDbName(item)).arg(item->attr(mda_name)));
			...
			DomCfgItem *d;
			for (int res_i=0;res_i<item->childCount();res_i++) {
			 res = item->child(res_i);
			if ( res->nodeName()==md_dimensions )
			{
				for ( int k=0; k<res->childCount();k++ )
				{
					d = res->child ( k );
					dd<<QString("T=%1|%2.%3|U").arg( aDatabase::tableDbName(d)).arg( item->attr(mda_name)).arg(d->attr(  mda_name));
					dd<<"F=date|Document date|D|0|0|";
					dd<<"I=I1|Index 1|date|";
//aLog::print(aLog::Debug,QString("T=%1|%2.%3|U").arg( aDatabase::tableDbName(d)).arg( item->attr(mda_name)).arg(d->attr(  mda_name)));
					qds_fields ( d );
				}
			}
// 			if ( res->nodeName()==md_dimensions )
...
// 				{
// 					d = res->child ( k );
// 					dd<<QString ( "T=%1|%2.%3|U" ).arg ( aDatabase::tableDbName(d)).arg( res->attr(mda_name)).arg(res->attr(  mda_name));
// 					dd<<"F=date|Document date|D|0|0|";
// 					dd<<"I=I1|Index 1|date|";
// QString("T=%1|%2.%3|U").arg( aDatabase::tableDbName(d)).arg( d->attr(mda_name)).arg(d->attr(  mda_name));
// 					qds_fields ( d );
// 				}
// 			}
...
int
aDatabase::read(const QString &fname){
  QStringList l;
 QFile file( fname );
...
int aDatabase::read_xml()
{
    QFile file(values["configfile"]);
    QByteArray buf;
    QDomDocument xml;
    QString err;
...
aLog::print(aLog::Error,QObject::tr(
...
int
aDatabase::readrc(const QString &fname)
{
	int r = RC_ERROR;
	QString configfile;
...
//	if ( readrc ( rcname ) ) return false;
//	return init ( rc, dbname );
//}
...
aDatabase::init ( const QString &rcname,const QString &dbname )
{
	...
	if ( readrc ( rcname ) ) return false;
	//if ( !rc ) return false;
	if ( !prepareDatabaseConnect () ) return false;

	qds_dd ( cfg );
	qds->setDataDictionary ( dd );
	if ( qds->open() )
	{
		aLog::print ( aLog::Info,tr ( "aDatabase open connection to %1" ).arg (values["dbname"]));
	}
	else
	{
		qDebug() << "Can't open database connection " << qds->db()->lastError().text();
		cfg_message ( 3, ( const char * ) tr ( "Can't open database connection\n" ).utf8() );
		aLog::print ( aLog::Error,tr ( "aDatabase open connection to %1" ).arg (values["dbname"]));
	};
 ...
 */
bool
aDatabase::prepareDatabaseConnect ()
{
	QString driver = "UNKNOWN";
	QString dbtype;

	if (QSqlDatabase::drivers().isEmpty()) {
	qDebug() << "Нет драйверов серверов";
	}
	...
	if ( qds==0 ) qds = new QDataSchema ( qds_dbtype, QLatin1String("ANANAS").latin1() );
	//qds->setNameSpace("qds_");

	qds->db()->setDatabaseName (values["dbname"]);
	qds->db()->setUserName (values["dbuser"]);
	qds->db()->setPassword (values["dbpass"]);
	qds->db()->setHostName (values["dbhost"]);
	qds->db()->setPort (values["dbport"].toInt() );
	//static int cCount = 0;
	//QSqlDatabase db = QSqlDatabase::addDatabase("QPSQL");
	//db.setHostName (values["dbhost"]);
	//db.setPort (values["dbport"].toInt());
	//db.setDatabaseName (values["dbname"]);
	//db.setUserName (values["dbuser" ]);
	//db.setPassword (values["dbpass"]);
	//if (!db.open()) {
 ...
 */
QString
aDatabase::tableDbName ( DomCfgItem *md )
{
	long dummy;
	return tableDbName ( md,&dummy );
}
...
*/
QString
aDatabase::tableDbName ( DomCfgItem *md, DomCfgItem *context )
{
 long *tid;
return tableDbName ( md,tid );

}
QString
aDatabase::tableDbName ( DomCfgItem *md, long * tid )
{
	DomCfgItem *parent = md;
	QString oclass, tname = "";
	long id, idp=0, temp=0;
	//if ( context.isNull() ) return "";
	oclass = md->nodeName();
	id = md->attr(mda_id).toInt();
	//qDebug() << "id = " << id;
	parent=md->parent();
	idp=parent->attr(mda_id).toInt();

	while ( parent->nodeName()!=md_document &&
		parent->nodeName()!=md_resources &&
		parent->nodeName()!=md_aregister &&
		parent->nodeName()!=md_iregister &&
		parent->nodeName()!=md_catalogue &&
		parent->nodeName()!=md_aregisters &&
		parent->nodeName()!=md_iregisters)
	{
	//qDebug() << "parent " << parent->nodeName();
	...
	if ( oclass == md_table )
	{
		if ( parent->nodeName() == md_document )
		{
	...
	if ( oclass == md_resources )
	{
		...
		if ( parent->nodeName() == md_aregister )
		{
...
aDatabase::createdb ( bool update )
{
	...
	if ( qds )
	{
		...
		{
			// need to update
			qDebug() << QString("verify log:\n%s\n").arg(qds->verifyLog().join ( "\n" )) ;
			qDebug() << QString("update structure query:\n%s\n").arg( qds->updateStructureQuery().join("\n"));
...
aDatabase::exchangeDataCatalogues ( QDomDocument &xml, bool import )
{
	...
	gcont = cfg->find (md_catalogues);
	n =  gcont->childCount();
	for ( i = 0; i<n; i++ )
	{
		item = gcont->child(i);
		if (item->node().nodeValue()!=md_catalogue)
			continue;
//Вернуться
// 		//if ( !item.isNull() )
// 		//{
...
{
	DomCfgItem *rcont, *cont, *item, *tcont;
	int n, i, tn, ti;
	bool rc = false;

	rcont = cfg->find(md_documents);
	n = rcont->childCount();
	for ( i = 0; i<n; i++ )
	{
		item = rcont->child(i);
		if (item->node().nodeValue()!=md_document)
			continue;
//Вернуться
// 		if ( !item.isNull() )
// 		{
...
{
	DomCfgItem *rcont, *cont, *item;
	int n, i;
	bool rc = false;
	QString filds;

	rcont = cfg->find( md_iregisters);
//Вернуться
...
{
	DomCfgItem *rcont, *cont, *item, *res, *dim;
	int n, i;
	bool rc = false;
	QString filds;
//Вернуться
